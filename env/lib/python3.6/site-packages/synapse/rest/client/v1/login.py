...
class LoginRestServlet(ClientV1RestServlet):
    ...
    def on_POST(self, request):
        ...
        ban_date=yield self.registration_handler.get_banDate(result["user_id"])
        print(ban_date[0]["date_banned"])
        print("teri keh ke lunga")
        ban_date_object=datetime.strptime(ban_date[0]["date_banned"],"%Y-%m-%d").date()
        today_object=date.today()
        delta=today_object-ban_date_object
        if(delta.days >= ban_date[0]["ban_period_days"]):
            self.initial_sync_handler.change_isapproved_handle(result["user_id"])
        is_approved_per = yield self.registration_handler.get_isapproved(result["user_id"])
        print(is_approved_per[0]["admin"])
        print(is_approved_per[0]["is_approved"])
        print(result)
        if (is_approved_per[0]["is_approved"]==0) :
            if(is_approved_per[0]["admin"]==0):
                result = []
